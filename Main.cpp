	...
	: TForm(Owner)
{
	...
	int openresult = sqlite3_open16(L".\\ClusterDB.sqlite",&Database);
	if (openresult != SQLITE_OK) {
		Application->MessageBox(L"Something wrong with table!", MB_OK);
...
void __fastcall TForm1::Button3Click(TObject *Sender)
{
	...
	bool EnableDec = CheckBox1 -> Checked;
	...
	Button1 -> Enabled = false;
	Edit1 -> Enabled = false;
	ReadThreadPtr = new ReadThread(false, FS,EnableDec, Database);
}
//---------------------------------------------------------------------------
...
void __fastcall TForm1::Button5Click(TObject *Sender)
{
	...
	if (result != SQLITE_OK) {
		Application->MessageBox(L"Error in DELETE FROM DB!",L"ERROR", MB_OK);
		return;
	}
...
void __fastcall TForm1::Button6Click(TObject *Sender)
{
	...
	while (true) {
		...
		if(result == SQLITE_DONE) break;
			...
			Application->MessageBox(L"Error in SELECT FROM DB!",L"ERROR", MB_OK);
			break;
		}
